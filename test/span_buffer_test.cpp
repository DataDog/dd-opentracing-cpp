...
TEST_CASE("span buffer") {
  ...
  SECTION("can write a single-span trace") {
    ...
    buffer->finishSpan(std::move(span), nullptr);
  ...
  SECTION("can write a multi-span trace") {
    ...
    buffer->finishSpan(std::move(childSpan), nullptr);
    buffer->finishSpan(std::move(rootSpan), nullptr);
    REQUIRE(writer->traces.size() == 1);
  ...
  SECTION("doesn't write an unfinished trace") {
    ...
    // Ok now we're done!
    buffer->finishSpan(std::move(childSpan2), nullptr);
    REQUIRE(writer->traces.size() == 1);
