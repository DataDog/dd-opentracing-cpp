...
size_t base64_expected_output_len(size_t input_len, bool padding) {
  ...
  const size_t expected_dst_len = (last_quantum * 4 / 3) + (last_quantum_size == 0 ? 0 : last_quantum_size + 1);
  return padding ? (expected_dst_len + 3) & ~3 : expected_dst_len;
}

void base64_generic(
  const char *b64, const unsigned char *src, size_t src_len, char *dst, size_t dst_len, bool padding) {
...
void base64(const unsigned char *src, size_t src_len, char *dst, size_t dst_len, bool padding = true) {
  static const char rfc4648_charset[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
...
void appendBase64Unpadded(std::string& destination, ot::string_view source) {
  ...
  base64(reinterpret_cast<const unsigned char*>(source.data()), source.size(), &*(destination.end() - base64_length), base64_length, padding);
}
